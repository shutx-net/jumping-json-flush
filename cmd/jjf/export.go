...
useful diagram - so xlsx and svg render it and "jjf validate" is where those
...
type exportFormat struct {
	...
	// screen": SVG is text, so it says false and may be written to a terminal,
	// exactly as the JSON of "jjf import" may.
	binary bool
	// accept reports why this format refuses doc, or nil when it accepts it.
	// Only ddl sets it: see the entry below for why xlsx and svg deliberately
	// do not.
...
func exportFormats() []exportFormat {
	return []exportFormat{
		...
		// The diagram format, and the first entry the terminal guard lets
		// through: SVG is text, so the question the binary field asks - would
		// this garble the screen - answers itself.
		//
		// It has no accept, for the reason the ddl entry below spells out:
		// internal/export/svg draws a foreign key with no target as a dashed
		// stub on purpose, so a document that contradicts itself is not
		// something this format has to refuse.
		{
		...
		// a useful diagram - internal/export/svg draws a foreign key with no
...
//
// The comparison is exact: XLSX and Svg are unknown formats, just as MySQL is
