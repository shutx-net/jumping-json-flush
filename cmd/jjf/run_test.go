...
// TestAnImportedDocumentIsOneTheDDLExporterAccepts pins the round trip this
// tool is built on: what "jjf import" writes, "jjf export ddl" must be willing
// to write back. Both halves already promise it separately - the importer
// validates what it writes, and design/ddl-export.md justifies every precheck
// by asking whether a document the importer produced could be in that state -
// but nothing was holding the two promises against each other.
//
// The two dumps below are the shapes that broke it, both of them real output
// from a real server. Neither warns on import, both used to be refused by the
// exporter afterwards, and the exit code the user saw was 2 for a document jjf
// had just written itself.
func TestAnImportedDocumentIsOneTheDDLExporterAccepts(t *testing.T) {
	tests := []struct {
		name    string
		dialect string
		dump    string
		...
		{
			// PostgreSQL makes an identity column and a serial column NOT NULL
			// whether or not the statement says so; measured on 16.13, all
			// three of these have attnotnull set.
			name:    "postgres identity and serial columns",
			dialect: "postgres",
		...
		{
			// What mysqldump 8.0.46 writes for a default it cannot spell in
			// the connection charset - an emoji, here - and what MySQL itself
			// accepts back.
			name:    "a mysql default written as a hexadecimal literal",
			dialect: "mysql",
			dump: "CREATE TABLE `emoji` (\n" +
				"  `w` varchar(5) NOT NULL DEFAULT 0xF09F8DA3\n" +
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := os.WriteFile(dump, []byte(tt.dump), 0o644); err != nil {
				t.Fatal(err)
			}
			doc := filepath.Join(dir, "doc.json")

			var stdout, stderr bytes.Buffer
			if code := run([]string{"import", tt.dialect, dump, "-database", "d", "-o", doc}, &stdout, &stderr); code != 0 {
				t.Fatalf("import = %d, want 0\nstderr: %s", code, &stderr)
			}
			if stderr.Len() != 0 {
				t.Errorf("import warned: %s", &stderr)
			}

			stdout.Reset()
			stderr.Reset()
			if code := run([]string{"validate", doc}, &stdout, &stderr); code != 0 || stderr.Len() != 0 {
				t.Errorf("validate = %d, want 0 with nothing on stderr\nstderr: %s", code, &stderr)
			}

			stdout.Reset()
			stderr.Reset()
			if code := run([]string{"export", "ddl", doc, "-o", filepath.Join(dir, "out.sql")}, &stdout, &stderr); code != 0 {
				t.Errorf("export ddl = %d, want 0: the exporter refused a document jjf wrote\nstderr: %s", code, &stderr)
			}
		})
	}
}
