...
func stopAtColumnConstraint(start int) func(*parser) bool {
	return func(p *parser) bool {
		...
		return p.i > start && tok.kind == kindWord && startsColumnConstraint(tok.text)
	}
}

// startsColumnConstraint reports whether w begins a column constraint. Every
// word here is reserved in PostgreSQL, so none of them can be the start of an
// expression that a DEFAULT would legitimately continue with.
func startsColumnConstraint(w string) bool {
	switch w {
	case "not", "null", "primary", "unique", "references", "check",
		"constraint", "generated", "collate", "deferrable", "initially", "default", "no":
		return true
	}
	return false
}
