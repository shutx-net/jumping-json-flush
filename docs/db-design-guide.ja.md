...
編集の種類ごとの実例 JSON。いずれのレシピも終わり方は同じである —
...
- **意味的整合性の検証。** 外部キー参照先の存在、テーブル名・カラム名の重複、キーや
  インデックスのカラムの存在、外部キー両端の型互換性、主キーの `nullable` 整合はいずれも
  未検査であり、書く側の責任である
- DDL / SQL の生成
- 実データベースへの接続、既存 DB からのスキーマ取り込み
