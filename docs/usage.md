...
Three formats are supported: `xlsx`, an Excel design document; `dot`, a Graphviz
entity relationship diagram; and `ddl`, a PostgreSQL DDL script. They share the
same contract.
...
- Leave `-o` out and the output goes **next to the input, with the extension
  ...
  `docs/db-design.xlsx`, `docs/db-design.dot` or `docs/db-design.sql`). `ddl` is
  the one format whose extension is not its name: a `.ddl` file is nothing, and
  calling the format `sql` would promise arbitrary SQL rather than one
...
- **`ddl` alone refuses a document that contradicts itself** (exit code 2). SQL a
  ...
  a useful workbook and a useful diagram — so `xlsx` and `dot` render it and
...
**The same input always produces a byte-identical `.xlsx`, `.dot` and `.sql`.** No
