...
- 想定しているのは **pg_dump 13 〜 18** と **MySQL 8.0 / 8.4** の出力であり、
  リポジトリに取り込んだ実ダンプで検証済みである（PostgreSQL はこの範囲の全メジャーが、
  MySQL は捕獲したすべての系列が、同一の文書にバイト単位で一致して取り込まれる）。
  ヘッダのバージョン表記を読み、範囲外のダンプは失敗ではなく警告にする。名乗る範囲は
  捕獲したダンプが覆う範囲ちょうどで、MySQL では範囲をメジャーで述べるので、範囲を
  広げるのは 9.x を捕獲したときである
...
の並び、`NULLS NOT DISTINCT`、`DEFERRABLE` は設計フォーマットに書き場所が無いため、
警告して捨てる。`-schema` で選ばなかったスキーマのものも捨てるが、そちらへ張られた
外部キーだけは実在する関係が失われるので報告する。

列が自分の文に書かれていないテーブルは、書かれている分から組み立てるのではなく、
報告したうえで丸ごと落とす。`CREATE TABLE child (LIKE parent)`、型付きテーブル、
パーティション、MySQL ではさらに `CREATE TABLE t LIKE other` とクエリから定義した
テーブルがこれにあたる。どちらのダンプツールもこれらを書かないので、手書き SQL の
話である。
