...
| `jjf: unsupported format "csv"; supported formats: xlsx, dot, svg, ddl` | an export format that does not exist | the formats are `xlsx`, `dot`, `svg` and `ddl` |
...
does. `xlsx`, `dot` and `svg` render the same document happily, because a
slightly broken document still makes a useful workbook and a useful diagram. See
